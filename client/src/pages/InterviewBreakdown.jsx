    ...
    return (
        <div className="relative flex flex-col items-center">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                {label && <span className="text-[10px] font-mono text-white/40 uppercase tracking-widest mt-1">{label}</span>}
            </div>
        </div>
    );
};
    ...
    return (
        <div className="space-y-2">
            <div className="flex justify-between items-center">
                <span className="text-sm text-white/70 font-medium truncate max-w-[70%]">{label}</span>
        ...
        >
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                    <h3 className="text-base font-medium text-white/90 leading-relaxed line-clamp-2">
                        {question}
                    </h3>
                    {analysis?.topic && (
                        <span className="inline-block mt-2 text-[10px] font-mono uppercase tracking-widest text-white/40 bg-white/5 px-3 py-1 rounded-full">
                            {analysis.topic}
                        </span>
                    )}
                </div>
                ...
                {open && (
                    ...
                    >
                        <div className="px-6 pb-6 space-y-4 border-t border-white/[0.06] pt-4">
                            {/* User Answer */}
                            <div className="bg-black/30 rounded-xl p-4 border-l-2 border-blue-500/40">
                                ...
                                <p className="text-white/60 text-sm italic leading-relaxed">"{answer || 'No response recorded'}"</p>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="bg-green-500/5 rounded-xl p-4 border border-green-500/10">
                                    ...
                                    <p className="text-sm text-white/70 leading-relaxed">{analysis?.feedback || 'N/A'}</p>
                                </div>
                                <div className="bg-purple-500/5 rounded-xl p-4 border border-purple-500/10">
                                    ...
                                    <p className="text-sm text-white/70 leading-relaxed">{analysis?.idealAnswer || 'N/A'}</p>
                                </div>
                            </div>
                        </div>
                    </motion.div>
...
const InterviewBreakdown = () => {
    ...
    return (
        <div className="min-h-screen relative flex flex-col selection:bg-white/20 pb-24 bg-black overflow-x-hidden font-sans">
            ...
            <div className="relative z-10 w-full max-w-[1400px] mx-auto px-6 md:px-12 pt-8">
                <motion.button
                    ...
                    className="flex items-center gap-2 text-xs font-mono text-white/40 hover:text-white/80 uppercase tracking-widest transition-colors group"
                >
            ...
            {/* HERO HEADER */}
            <div className="relative z-10 w-full max-w-[1400px] mx-auto px-6 md:px-12 pt-8 pb-12 border-b border-white/[0.05]">
                <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                    <div className="flex items-center gap-3 text-xs font-mono text-white/40 mb-4 uppercase tracking-[0.2em]">
                    ...
                    <h1 className="text-4xl md:text-6xl font-heading font-bold text-white tracking-tighter mb-3">
                        {interview.role}
                    </h1>
                    <div className="flex flex-wrap items-center gap-3 mt-4">
                        <span className="px-4 py-1.5 text-[10px] font-mono uppercase tracking-widest rounded-full border border-white/10 text-white/50 bg-white/[0.03]">
                    ...
                    >
                        <div className="lg:sticky lg:top-8 space-y-8">
                            ...
                            <div className={`${STYLES.glass_card} p-8 flex flex-col items-center`}>
                                <div className="text-[10px] font-mono text-white/40 uppercase tracking-widest mb-6">Overall Rating</div>
                                ...
                                <div className="w-full grid grid-cols-2 gap-3 mt-8">
                                    <div className="bg-white/[0.03] rounded-xl p-4 text-center border border-white/[0.06]">
                                        <div className="text-[9px] font-mono text-white/30 uppercase tracking-widest mb-1">Technical</div>
                                    ...
                                    <div className="bg-white/[0.03] rounded-xl p-4 text-center border border-white/[0.06]">
                                        <div className="text-[9px] font-mono text-white/30 uppercase tracking-widest mb-1">Communication</div>
                            ...
                            {questionAnalysis.length > 0 && (
                                <div className={`${STYLES.glass_card} p-6`}>
                                    <div className="text-[10px] font-mono text-white/40 uppercase tracking-widest mb-5 flex items-center gap-2">
                            ...
                            {feedback.summary && (
                                <div className={`${STYLES.glass_card} p-6`}>
                                    <div className="text-[10px] font-mono text-white/40 uppercase tracking-widest mb-4 flex items-center gap-2">
                                        <Brain className="w-3 h-3" /> AI Summary
                                    </div>
                                    <p className="text-sm text-white/60 leading-relaxed">{feedback.summary}</p>
                                </div>
                            )}
                    ...
                    >
                        ...
                        >
                            ...
                            <Button
                                onClick={() => navigate('/history')}
                                variant="ghost"
                                className="text-white/40 hover:text-white font-mono text-xs uppercase tracking-widest py-3 px-8 rounded-full"
                            >
                                Back to Archives
                            </Button>
                        </motion.div>
